<template>
  <div class="pt-12 pb-15 content-container add-container">
...
import categoryCard from '~/components/new-advertisement/category-card';
...
.advertisement__form-category-active{
  &::after{
    background-image: url('~/assets/img/arrow-dark-right.png');
  }
}
.advertisement__form-input{
  max-width: 680px;
}
.advertisement__category-item{
  width: 30%;
  height: 100%;
}
.advertisement__form-box{
  width: 24%;
  height: 120px;
  img{
    width: 159px;
...
.advertisement__form-photo--active{
  padding: 0;
  position: relative;
  &:hover{
    &::after{
...
.advertisement__form-elements{
  display: grid;
  grid-template-columns: repeat(4, auto);
  gap: 16px;
  justify-content: center;
}
.advertisement__form-price{
  max-width: 300px;
}
.advertisement__form-currency{
  max-width: 100px;
}
